...
No shared packages, no tests. Home supports food/exercise logging, saved meals/history, water quick-add, and header-controlled date/month navigation. Weight supports local weigh-in logging, onboarding baseline summary/chart context, summary/history, chart trends, delete undo, and edge states. Water has daily logging for selected date plus a local 7-day trend and grouped history. Backend proxy deployment is configured for Fly.io; backend observability remains shallow and intentionally keeps OpenRouter checks outside `/health`.
